(function(){
  ...
  const DEFAULT_SKUS = ['1001','1002','1005','1008','1012','1016','2001','3001'];
  ...
  function render(){
    ...
    return /*html*/`
<div class="breadcrumb"><span>Settings</span><span class="crumb-dot">›</span><span>Reorder points</span></div>
...
<div class="page-head">
  ...
  <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;justify-content:flex-end;">
    <button class="ghost-btn" id="btnResetDefaults" ${readOnly ? 'disabled' : ''}>Reset to defaults</button>
...
<div class="hint" style="margin-top:12px;">
  ...
  function init(){
    ...
    const items = (store.items || []).filter(it => DEFAULT_SKUS.includes(String(it.sku||'').trim()));
    ...
    function resetDefaults(){
      if (readOnly){ toast('Only Owner can edit reorder points'); return; }
      store.settings.reorderPoints = Object.assign({}, store.settings.reorderPoints, defaultRPs());
      PD.saveStore && PD.saveStore(store);
      renderRows();
