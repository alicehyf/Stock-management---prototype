(function(){
  ...
  function buildApprovalChain(total, settings){
    ...
    return levels.map((lvl, idx)=>{
      ...
      if (!required) return { role, status:'Skipped' };
      if (!hasPending){
        hasPending = true;
        return { role, status:'Pending' };
      }
      return { role, status:'Blocked' };
    });
  }
    ...
    function renderPODetail(po){
      ...
      if (approvalActionsEl){
        ...
        if (!approvalsEnabled){
          ...
          approvalActionsEl.innerHTML = '<div class="hint">Rejected. Update the PO and resubmit if needed.</div>';
        } else if (isDraft){
