      ...
      <table class="table">
        ...
        <tbody id="apTbody">
          ${rows.map(pr=>{
            ...
            const disabled = (!canApprove || pr.status!=='Pending approval') ? 'disabled' : '';
            return `
            <tr class="row-click" data-type="PR" data-id="${pr.id}">
              ...
              <td>
                <div style="display:flex;gap:8px;justify-content:flex-end;flex-wrap:wrap">
                  ...
                  <button class="btn sm primary" ${disabled} data-act="approve" data-type="PR" data-id="${pr.id}">Approve</button>
                  <button class="btn sm" ${disabled} data-act="reject" data-type="PR" data-id="${pr.id}">Reject</button>
                </div>
              </td>
            </tr>`;
      ...
      <table class="table">
        ...
        <tbody id="apTbody">
          ${rows.map(po=>{
            ...
            const disabled = (!canApprove || po.status!=='Pending approval') ? 'disabled' : '';
            return `
            <tr class="row-click" data-type="PO" data-id="${po.id}">
              ...
              <td>
                <div style="display:flex;gap:8px;justify-content:flex-end;flex-wrap:wrap">
                  ...
                  <button class="btn sm primary" ${disabled} data-act="approve" data-type="PO" data-id="${po.id}">Approve step</button>
                  <button class="btn sm" ${disabled} data-act="reject" data-type="PO" data-id="${po.id}">Reject</button>
                </div>
              </td>
            </tr>`;
...
`;
  ...
  function approveRecord(type, id){
    const can = PD.can ? PD.can(type==='PR' ? 'approve_pr' : 'approve_po') : (PD.getRole?.() === 'owner');
    if (!can){ toast('No permission. Please wait for approval.'); return; }

    const store = PD.getStore();
    if (type==='PR'){
      const pr = (store.prs||[]).find(p=>p.id===id);
      if (!pr){ toast('PR not found'); return; }
      if (pr.status !== 'Pending approval'){ toast('PR not pending'); return; }
      (pr.approvals||[]).forEach(ap=>{
      ...
      pr.activity.unshift({ at: PD.nowISO(), text:'Approved (from inbox)' });
      PD.saveStore(store);
      toast('Approved');
    ...
    // PO: approve one step at a time
    const po = (store.pos||[]).find(p=>p.id===id);
    if (!po){ toast('PO not found'); return; }
    if (po.status !== 'Pending approval'){ toast('PO not pending'); return; }
    po.approvals = po.approvals || buildApprovalChain(poTotal(po), store.settings);
    const idx = firstPendingIdx(po.approvals);
    if (idx < 0){
      // fallback
      po.status = 'Approved';
      po.approvedAt = PD.nowISO();
      po.activity = po.activity || [];
      po.activity.unshift({ at: PD.nowISO(), text:'PO approved (completed)' });
    } else {
      po.approvals[idx].status = 'Approved';
      po.approvals[idx].at = PD.nowISO();
      po.approvals[idx].by = 'Owner';
      po.activity = po.activity || [];
      po.activity.unshift({ at: PD.nowISO(), text:`Approved step: ${po.approvals[idx].role}` });
  ...
  function openReject(type, id){
    const can = PD.can ? PD.can(type==='PR' ? 'reject_pr' : 'reject_po') : (PD.getRole?.() === 'owner');
    if (!can){ toast('No permission. Please wait for approval.'); return; }
  ...
  function confirmReject(){
    ...
    if (type==='PR'){
      const pr = (store.prs||[]).find(p=>p.id===id);
      if (!pr){ toast('PR not found'); closeReject(); return; }
      pr.status='Rejected';
      pr.rejectedAt=PD.nowISO();
      if (pr.approvals && pr.approvals.length){
        const pend = pr.approvals.find(a => String(a.status||'').toLowerCase()==='pending') || pr.approvals[0];
        pend.status='Rejected';
        pend.at=PD.nowISO();
        pend.by='Owner';
        if (note) pend.note = note;
      }
      pr.activity = pr.activity || [];
      pr.activity.unshift({ at: PD.nowISO(), text:'Rejected (from inbox)' + (note?(': '+note):'') });
      PD.saveStore(store);
      closeReject();
      toast('Rejected');
      window.RouterApp?.render?.();
    ...
    const po = (store.pos||[]).find(p=>p.id===id);
    if (!po){ toast('PO not found'); closeReject(); return; }
    po.status='Rejected';
    po.rejectedAt=PD.nowISO();
    po.approvals = po.approvals || buildApprovalChain(poTotal(po), store.settings);
    const idx = firstPendingIdx(po.approvals);
    if (idx >= 0){
    ...
    po.activity = po.activity || [];
    po.activity.unshift({ at: PD.nowISO(), text:'Rejected (from inbox)' + (note?(': '+note):'') });
    PD.saveStore(store);
    closeReject();
    toast('Rejected');
    window.RouterApp?.render?.();
