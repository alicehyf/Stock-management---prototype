  ...
  <div class="split">
    <div class="card panel col-8">
      ...
      <div class="btn-row">
        ...
        <button class="ghost-btn" id="btnWithdrawPR" title="Withdraw this approval request so you can edit the PR and resubmit.">Withdraw request</button>
        <button class="btn-secondary" id="btnRevisePR" title="Make changes and submit again." style="height:40px;border-radius:12px;">Edit & resubmit</button>
...
<div class="modal-overlay" id="prItemModal">
  ...
  async function init(){
    ...
    function renderPRDetail(pr){
      ...
      // Items
      prItemsTbody.innerHTML = (pr.items||[]).map((it, idx)=>{
        const uom = it.uom || 'g';
        const qtyVal = Number(it.qty||0);
        const sub = qtyVal * Number(it.estUnitPrice||0);
        const qtyCell = isDraft
          ? `<div class="qty-input-wrap">
               <input class="inline-input pr-qty-input" data-idx="${idx}" type="number" min="0" step="1" value="${qtyVal}" />
               <span class="mini-muted uom-suffix">${uom}</span>
             </div>`
          : `<span class="mono">${qtyVal.toLocaleString('en-US')} ${uom}</span>`;
        return `
          <tr>
            <td style="font-weight:950;">${it.name}</td>
            <td class="mono cell-muted">${it.sku}</td>
            <td class="cell-muted">${uom}</td>
            <td class="qty-cell">${qtyCell}</td>
            <td class="cell-muted">${it.supplier || '—'}</td>
            <td class="mono">${formatIDR(it.estUnitPrice||0)}</td>
            <td class="right mono" id="prSub_${idx}">${formatIDR(sub)}</td>
          </tr>
        `;
      }).join('') || '<tr><td colspan="7" class="mini-muted">No items yet. Add items to this requisition.</td></tr>';

      // Enable inline quantity edits for draft PRs
      if (isDraft){
        prItemsTbody.querySelectorAll('.pr-qty-input').forEach(inp=>{
          inp.addEventListener('input', ()=>{
            const idx = Number(inp.getAttribute('data-idx'));
            const store2 = PD.getStore();
            const pr2 = (store2.prs||[]).find(p=>p.id===currentPRId);
            if (!pr2 || !pr2.items || !pr2.items[idx]) return;

            const v = Math.max(0, Number(inp.value||0));
            pr2.items[idx].qty = v;
            PD.saveStore(store2);

            const sub = v * Number(pr2.items[idx].estUnitPrice||0);
            const subEl = document.getElementById('prSub_' + idx);
            if (subEl) subEl.textContent = formatIDR(sub);

            const totalNow = calcPRTotal(pr2);
            const kvTotalEl = document.getElementById('kvTotal');
            if (kvTotalEl) kvTotalEl.textContent = formatIDR(totalNow);

            // Keep list view totals in sync (even if hidden)
            renderPRList();
          });
        });
      }
      ...
      const btnWithdraw = document.getElementById('btnWithdrawPR');
      const btnRevise = document.getElementById('btnRevisePR');
      ...
      if (btnWithdraw){
        btnWithdraw.style.display = isPending ? 'inline-flex' : 'none';
        btnWithdraw.disabled = !isPending;
      }
      if (btnRevise){
        btnRevise.style.display = isRejected ? 'inline-flex' : 'none';
        btnRevise.disabled = !isRejected;
      }
    ...
    function ensureApprovalSteps(pr, store){
      ...
      const total = calcPRTotal(pr);
      // Always rebuild chain when (re)submitting from Draft, Withdrawn, or when missing approvals
      if (!Array.isArray(pr.approvals) || pr.approvals.length===0 || pr.status==='Draft'){
        pr.approvals = (PD.buildApprovalChain ? PD.buildApprovalChain('PR', total, store.settings) : (pr.approvals||[]));
      }
      // Backfill for legacy approvals objects
    ...
    function submitPR(){
      ...
      if (enabled && (pr.approvals||[]).length){
        ...
        pr.approvals = (PD.buildApprovalChain ? PD.buildApprovalChain('PR', calcPRTotal(pr), store.settings) : pr.approvals);
    ...
    function approvePR(){
      ...
      const pr = (store.prs||[]).find(p=>p.id===currentPRId);
      if (!pr) return;
      if (!PD.can || !PD.can('approve_pr', { docType:'PR', doc: pr })){
        toast('No permission. Please wait for approval.');
        return;
      }
      if (pr.status !== 'Pending approval'){ toast('PR not pending'); return; }

      pr.approvals = pr.approvals || (PD.buildApprovalChain ? PD.buildApprovalChain('PR', calcPRTotal(pr), store.settings) : []);
      const idx = PD.firstPendingIdx ? PD.firstPendingIdx(pr.approvals) : (pr.approvals||[]).findIndex(a=>a && a.status==='Pending');
      if (idx < 0){ toast('No pending step'); return; }

      pr.approvals[idx].status = 'Approved';
      pr.approvals[idx].at = PD.nowISO();
      pr.approvals[idx].by = PD.getRoleName ? PD.getRoleName(PD.getRole()) : (PD.getRole?PD.getRole():'');
      if (PD.unblockNext) PD.unblockNext(pr.approvals);

      pr.activity = pr.activity || [];
      pr.activity.unshift({ at: PD.nowISO(), text:`Approved step: ${pr.approvals[idx].roleName || pr.approvals[idx].role}` });

      const remaining = PD.hasRemainingApproval ? PD.hasRemainingApproval(pr.approvals) : (pr.approvals||[]).some(a=>a && (a.status==='Pending'||a.status==='Blocked'));
      if (!remaining){
        pr.status = 'Approved';
        pr.approvedAt = PD.nowISO();
        pr.activity.unshift({ at: pr.approvedAt, text:'PR approved (completed)' });
      }

      PD.saveStore(store);
      toast(pr.status==='Approved' ? 'Approved' : 'Approval progressed');
      renderPRDetail(pr);
      renderPRList();
    }
    ...
    function rejectPR(){
      ...
      const pr = (store.prs||[]).find(p=>p.id===currentPRId);
      if (!pr) return;
      if (!PD.can || !PD.can('reject_pr', { docType:'PR', doc: pr })){
        toast('No permission. Please wait for approval.');
        return;
      }
      if (pr.status !== 'Pending approval'){ toast('PR not pending'); return; }
      ...
      pr.approvals[idx].status = 'Rejected';
      pr.approvals[idx].at = PD.nowISO();
      pr.approvals[idx].by = PD.getRoleName ? PD.getRoleName(PD.getRole()) : (PD.getRole?PD.getRole():'');
      if (note.trim()) pr.approvals[idx].note = note.trim();

      pr.status = 'Rejected';
      pr.rejectedAt = PD.nowISO();
      pr.activity = pr.activity || [];
      pr.activity.unshift({ at: pr.rejectedAt, text:'Rejected' + (note.trim()?(': '+note.trim()):'') });
    ...
    function withdrawPR(){
      ...
      const pr = (store.prs||[]).find(p=>p.id===currentPRId);
      if (!pr) return;
      if (pr.status !== 'Pending approval'){ toast('PR is not pending'); return; }
      const ok = window.confirm ? window.confirm('Withdraw this approval request? You can edit the PR and submit again.') : true;
      if (!ok) return;

      pr.status = 'Draft';
      pr.submittedAt = null;
      // Reset approval steps so a fresh chain is created on resubmission
      pr.approvals = [];
      pr.activity = pr.activity || [];
      pr.activity.unshift({ at: PD.nowISO(), text:'Approval request withdrawn (back to Draft)' });
    ...
    function reviseRejectedPR(){
      const store = PD.getStore();
      const pr = (store.prs||[]).find(p=>p.id===currentPRId);
      if (!pr) return;
      if (pr.status !== 'Rejected'){ toast('PR is not rejected'); return; }

      pr.status = 'Draft';
      pr.submittedAt = null;
      pr.approvals = [];
      pr.activity = pr.activity || [];
      pr.activity.unshift({ at: PD.nowISO(), text:'Reopened for changes (after rejection)' });
    ...
    document.getElementById('btnWithdrawPR')?.addEventListener('click', withdrawPR);
    document.getElementById('btnRevisePR')?.addEventListener('click', reviseRejectedPR);
