  ...
  function seedStore(){
    return {
      ...
      settings: {
        ...
        // Configurable roles and permissions (editable in Settings)
        roles: [
                    { id:'staff', name:'Staff', tag:'Limited', permissions:{}},
          { id:'outlet_manager', name:'Outlet manager', tag:'Ops', permissions:{
            approve_pr:true, reject_pr:true, approve_po:true, reject_po:true
          }},
          { id:'finance', name:'Finance', tag:'Finance', permissions:{
            approve_pr:true, reject_pr:true, approve_po:true, reject_po:true
          }},
          { id:'owner', name:'Owner', tag:'Admin', permissions:{ admin_all:true, approve_all:true, reject_all:true }},
        ],
        // Approval workflow steps (docType + minTotal)
        approvalSteps: [
          // PR workflow
          { id:'pr_1', docType:'PR', roleId:'outlet_manager', minTotal:0 },
          { id:'pr_2', docType:'PR', roleId:'finance', minTotal:5000000 },
          { id:'pr_3', docType:'PR', roleId:'owner', minTotal:20000000 },
          // PO workflow
          { id:'po_1', docType:'PO', roleId:'outlet_manager', minTotal:0 },
          { id:'po_2', docType:'PO', roleId:'finance', minTotal:5000000 },
          { id:'po_3', docType:'PO', roleId:'owner', minTotal:20000000 },
        ],
        thresholds: [
...
inventory: {
  ...
  // Nearest expiry per SKU (used by Ingredients + Stock Count pages)
  // Empty / '—' means expiry is not tracked for that item.
  expiryBySku: {
    "1001": "2026-06-30",
    "1002": "2027-01-10",
    "1005": "2026-05-15",
  ...
  function postInventoryFromReceiving(receivingId){
    ...
    (rcv.items||[]).forEach(it=>{
      ...
      store.inventory.movements[sku].unshift({
        // High-resolution timestamp for correct cross-item ordering in UI
        at: nowISO(),
        date: postedDate,
        type: 'IN',
        reference: rcv.id,
  ...
  function normalizeStore(store){
    ...
    // Legacy compatibility: older versions stored store.user.role only
    if (!store.user.roleId){
      store.user.roleId = (store.user.role === 'owner') ? 'owner' : (store.user.role ? String(store.user.role) : 'staff');
    }
    // Keep legacy field in sync for older pages
    store.user.role = store.user.roleId;
    ...
    if (!Array.isArray(store.settings.roles)) store.settings.roles = seedStore().settings.roles;
    if (!Array.isArray(store.settings.approvalSteps)) store.settings.approvalSteps = seedStore().settings.approvalSteps;

    // Prune permission keys that are not currently implemented in the prototype UI/pages.
    // Keeps storage clean and prevents "ghost" permissions from showing up in exports/debugging.
    const ACTIVE_PERMS = new Set([
      'create_po','create_receiving','manage_reorder_points',
      'approve_pr','reject_pr','approve_po','reject_po',
      'approve_all','reject_all','admin_all'
    ]);
    (store.settings.roles||[]).forEach(r=>{
      if (!r) return;
      const p = r.permissions || {};
      const next = {};
      Object.keys(p).forEach(k=>{
        if (ACTIVE_PERMS.has(k) && p[k]) next[k] = true;
      });
      r.permissions = next;
    });


    // Normalize approvals to include roleId + roleName (needed for step-based approvals)
    try{
      const roleNameToId = {};
      (store.settings.roles||[]).forEach(r=>{ if (r && r.name) roleNameToId[String(r.name)] = String(r.id||''); });
      const fixDoc = (doc)=>{
        (doc && doc.approvals || []).forEach(a=>{
          if (!a) return;
          if (!a.roleId && a.role){
            const rid = roleNameToId[String(a.role)] || '';
            if (rid) a.roleId = rid;
          }
          const rid2 = String(a.roleId||'');
          const rdef = (store.settings.roles||[]).find(x=>String(x.id)===rid2);
          if (!a.roleName) a.roleName = rdef ? rdef.name : (a.role || '');
          if (!a.role && a.roleName) a.role = a.roleName;
        });
      };
      (store.prs||[]).forEach(fixDoc);
      (store.pos||[]).forEach(fixDoc);
    }catch(e){}
    ...
    if (!store.inventory.expiryBySku) store.inventory.expiryBySku = seedStore().inventory.expiryBySku;
    if (!store.inventory.movements) store.inventory.movements = seedStore().inventory.movements;

    // Backfill any missing expiry entries without overwriting user changes.
    try{
      const seedExp = (seedStore().inventory && seedStore().inventory.expiryBySku) ? seedStore().inventory.expiryBySku : {};
      store.inventory.expiryBySku = store.inventory.expiryBySku || {};
      Object.keys(seedExp).forEach(k => {
        if (store.inventory.expiryBySku[k] == null) store.inventory.expiryBySku[k] = seedExp[k];
      });
    }catch(e){}
    ...
    // Backfill movement timestamps so UI can sort movements within the same date.
    ensureMovementTimestamps(store);

    return store;
  }

  function ensureMovementTimestamps(store){
    store.inventory = store.inventory || {};
    store.inventory.movements = store.inventory.movements || {};
    const mv = store.inventory.movements;
    Object.keys(mv).forEach(sku=>{
      const arr = mv[sku];
      if (!Array.isArray(arr)) return;
      const skuOffsetMs = (parseInt(String(sku).replace(/\D/g,''),10) || 0) % 50 * 1000;
      arr.forEach((m, idx)=>{
        if (!m || m.at) return;
        const d = (m.date && /^\d{4}-\d{2}-\d{2}$/.test(m.date)) ? m.date : null;
        const base = d ? new Date(d + 'T23:59:59.000Z') : new Date();
        // Newest entries are usually earlier in the array (unshift). Preserve that order.
        const at = new Date(base.getTime() - idx*60000 - skuOffsetMs).toISOString();
        m.at = at;
      });
    });
  }
  ...
  function createStockIssue(payload){
    ...
    // Apply OUT movements
    // Use a monotonically increasing timestamp so cross-item ordering is correct
    const baseMs = Date.now();
    issue.lines.forEach((line, i)=>{
      const sku = line.sku;
      const meta = getUomMeta(store, sku);
      const qtyBase = toBaseQty(store, sku, line.qty, line.uom||meta.baseUom);
      if (!(qtyBase>0)) return;
  ...
  function computeOnOrderBySku(store){
    const map = {};
    const pos = Array.isArray(store.pos) ? store.pos : [];
    pos.forEach(po=>{
      const st = String(po.status||'').toLowerCase();
      // Only count quantities that have been formally placed (exclude Draft)
      if (st === 'draft') return;
      if (['received','cancelled','canceled','void','closed','rejected'].includes(st)) return;
      const items = Array.isArray(po.items) ? po.items : [];
      items.forEach(it=>{
        const sku = String(it.sku||'').trim();
        if (!sku) return;
        const meta = getUomMeta(store, sku);
        const orderedBase = toBaseQty(store, sku, it.orderedQty||0, it.uom||meta.baseUom);
        const receivedBase = toBaseQty(store, sku, it.receivedQty||0, it.uom||meta.baseUom);
        const openBase = Math.max(0, orderedBase - receivedBase);
        if (!(openBase > 0)) return;
        map[sku] = (map[sku]||0) + openBase;
      });
    });
    return map;
  }
  function getSuggestedOrders(opts){
    ...
    const onOrderMap = computeOnOrderBySku(store);
    ...
    Object.keys(rpMap||{}).forEach(sku=>{
      ...
      const onOrder = Number(onOrderMap[sku]||0);
      const effective = onHand + onOrder;
      if (effective >= rp) return;
      const target = Math.ceil(rp * buffer);
      const suggestionBase = Math.max(0, target - effective);
      const meta = getUomMeta(store, sku);
    ...
    out.sort((a,b)=>{
      const ra = (a.effectiveOnHandBase ?? a.onHandBase) / Math.max(1, a.reorderPointBase);
      const rb = (b.effectiveOnHandBase ?? b.onHandBase) / Math.max(1, b.reorderPointBase);
      return ra - rb;
    });
    return out;
  }
  ...
  function getRole(){
    const store = getStore();
    return (store.user && store.user.roleId) ? String(store.user.roleId) : 'staff';
  }

  function setRole(roleId){
    const store = getStore();
    store.user = store.user || {};
    const rid = String(roleId||'').trim() || 'staff';
    const roles = (store.settings && Array.isArray(store.settings.roles)) ? store.settings.roles : seedStore().settings.roles;
    const exists = roles.some(r=>r && String(r.id)===rid);
    store.user.roleId = exists ? rid : 'staff';
    // legacy for backward compatibility
    store.user.role = store.user.roleId;
    saveStore(store);
    return store.user.roleId;
  }

  function getRoleDef(roleId){
    const store = getStore();
    const rid = String(roleId||getRole()||'staff');
    const roles = (store.settings && Array.isArray(store.settings.roles)) ? store.settings.roles : [];
    return roles.find(r=>r && String(r.id)===rid) || roles.find(r=>r && String(r.id)==='staff') || { id: rid, name: rid, permissions:{} };
  }

  function getRoleName(roleId){
    const r = getRoleDef(roleId);
    return r && r.name ? String(r.name) : String(roleId||'');
  }

  function roleNameToIdMap(settings){
    const map = {};
    (settings && settings.roles || []).forEach(r=>{
      if (!r) return;
      if (r.name) map[String(r.name)] = String(r.id||'');
    });
    return map;
  }

  function buildApprovalChain(docType, total, settings){
    const s = settings || {};
    const steps = (Array.isArray(s.approvalSteps) && s.approvalSteps.length) ? s.approvalSteps : seedStore().settings.approvalSteps;
    const roles = (Array.isArray(s.roles) && s.roles.length) ? s.roles : seedStore().settings.roles;
    const ridToName = {};
    roles.forEach(r=>{ if(r && r.id) ridToName[String(r.id)] = String(r.name||r.id); });

    const t = Number(total||0);
    const list = steps.filter(st=>st && String(st.docType||'').toUpperCase()===String(docType||'').toUpperCase());

    let hasPending = false;
    return list.map((st, idx)=>{
      const min = Number(st.minTotal||0);
      const required = t >= min;
      const roleId = String(st.roleId||'');
      const roleName = ridToName[roleId] || roleId || ('Step ' + (idx+1));
      const rule = (min>0 ? ('If total ≥ IDR ' + Math.round(min).toLocaleString('id-ID')) : 'Required');

      if (!required){
        return { roleId, roleName, role: roleName, status:'Skipped', rule };
      }
      if (!hasPending){
        hasPending = true;
        return { roleId, roleName, role: roleName, status:'Pending', rule };
      }
      return { roleId, roleName, role: roleName, status:'Blocked', rule };
    });
  }

  function firstPendingIdx(approvals){
    return (approvals||[]).findIndex(a=>a && String(a.status||'')==='Pending');
  }
  ...
  function pendingRoleIdForDoc(doc, settings){
    const pend = (doc && doc.approvals || []).find(a=>a && String(a.status||'')==='Pending');
    if (!pend) return '';
    if (pend.roleId) return String(pend.roleId);
    const map = roleNameToIdMap(settings||{});
    return map[String(pend.role||pend.roleName||'')] || '';
  }
  ...
  function can(action, ctx){
    ...
    if (perms.admin_all || roleId==='owner') return true;

    // Approval actions: enforce step ownership unless the role has approve_all/reject_all.
    if (act==='approve_pr' || act==='reject_pr' || act==='approve_po' || act==='reject_po'){
      if (!(perms[act] || perms.approve_all || perms.reject_all)) return false;
      const doc = ctx && ctx.doc;
      if (!doc) return !!perms[act];
      if (String(doc.status||'')!=='Pending approval') return false;
      const pendingRid = pendingRoleIdForDoc(doc, store.settings);
      if (!pendingRid) return false;
      if (pendingRid===roleId) return true;
      if (act.startsWith('approve') && perms.approve_all) return true;
      if (act.startsWith('reject') && perms.reject_all) return true;
      return false;
    }
